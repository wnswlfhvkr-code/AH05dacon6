...
class _F10DerivedFeaturePipeline(PreprocessingPipeline):
    """유전자별 변이 이벤트 수를 fold-train 지지도 필터 후 생성합니다."""
    ...
    def __init__(self, min_feature_support: int = 2, **_: object) -> None:
        ...
        self.steps = ("결측·WT 정규화", "fold-train 지지도 필터", "유전자별 변이 이벤트 수")
...
class EMF10PreprocessingPipeline(PreprocessingPipeline):
    """EMV45를 베이스로 F10 전용 파생 피처를 결합합니다."""
    ...
    def __init__(self, **parameters: object) -> None:
        ...
        self.derived_pipeline = _F10DerivedFeaturePipeline(**parameters)
    ...
    def _combine(v45: pd.DataFrame, derived: pd.DataFrame) -> pd.DataFrame:
        ...
        prefixed.columns = [f"F10__{column}" for column in prefixed.columns]
    ...
    def summary(self) -> dict[str, int]:
        ...
        result.update({
            ...
            "f10_derived_features": self.derived_feature_count_,
