"""EMV45 베이스에 F01~F19의 고유 파생 피처를 모두 결합한 F20."""
...
class EMF20PreprocessingPipeline(PreprocessingPipeline):
    """V45는 한 번만 유지하고 F01~F19 전용 피처를 모두 추가합니다."""
    ...
    def __init__(
        ...
        **parameters: object,
        ...
        unknown = set(overrides) - set(DERIVED_PIPELINE_CLASSES)
        if unknown:
        ...
        for feature_name, pipeline_class in DERIVED_PIPELINE_CLASSES.items():
        ...
        self.derived_feature_counts_: dict[str, int] = {
            name: 0 for name in DERIVED_PIPELINE_CLASSES
        }
        self.steps = (
            "EMV45 베이스 피처 1회 생성",
            "F01~F15 비레이블 파생 피처",
    ...
    def _combine(
        ...
        parts = [v45]
        for feature_name in DERIVED_PIPELINE_CLASSES:
