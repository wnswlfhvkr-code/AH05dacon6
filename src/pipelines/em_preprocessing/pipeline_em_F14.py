...
def _matches(token: str) -> bool:
    ...
    return bool(
        ...
        and not _NONSENSE.search(token)
    )


class _F14DerivedFeaturePipeline(PreprocessingPipeline):
    """fold-train 지지도를 통과한 유전자별 missense 변이 여부를 생성합니다."""
    ...
    def __init__(self, min_feature_support: int = 2, **_: object) -> None:
        ...
        self.steps = ("결측·WT 정규화", "consequence 판정", "유전자별 missense 여부")
    ...
    def _build_features(self, features: pd.DataFrame) -> pd.DataFrame:
        ...
        matrix.columns = [f"gene_missense__{gene}" for gene in self.selected_genes_]
        return matrix

    def fit(self, features: pd.DataFrame, labels: pd.Series) -> "EMF14PreprocessingPipeline":
        ...
        if not self.selected_genes_:
            raise ValueError("missense 지지도를 만족하는 유전자가 없습니다.")
    ...
    def summary(self) -> dict[str, int]:
        ...
        result.update({
            "missense_gene_features": len(self.selected_genes_),
            "dropped_gene_features": len(self.gene_columns) - len(self.selected_genes_),
        })
        return result


class EMF14PreprocessingPipeline(PreprocessingPipeline):
    """EMV45를 베이스로 F14 전용 파생 피처를 결합합니다."""
    ...
    def __init__(self, **parameters: object) -> None:
        ...
        self.derived_pipeline = _F14DerivedFeaturePipeline(**parameters)
    ...
    def _combine(v45: pd.DataFrame, derived: pd.DataFrame) -> pd.DataFrame:
        ...
        prefixed.columns = [f"F14__{column}" for column in prefixed.columns]
        return pd.concat([v45, prefixed], axis=1)

    def fit(
        self, features: pd.DataFrame, labels: pd.Series
    ) -> "EMF14PreprocessingPipeline":
    ...
    def summary(self) -> dict[str, int]:
        ...
        result.update({
            ...
            "f14_derived_features": self.derived_feature_count_,
