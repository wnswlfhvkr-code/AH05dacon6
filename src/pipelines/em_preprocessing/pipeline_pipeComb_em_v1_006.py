"""pipeComb_em_v1_006: JSJ9·EMV45 OOF stacking 전용 분리 피처."""
...
@dataclass
class OOFStackingFeatureBundle:
...
class PipeCombEMV1006PreprocessingPipeline(PreprocessingPipeline):
    """JSJ9 텍스트와 EMV45 트리 피처를 모델별 뷰로 분리합니다."""

    name = "pipeComb_em_v1_006"
    evaluation_folds = 5
    ...
    def _bundle(self, text_bundle, tree: pd.DataFrame) -> OOFStackingFeatureBundle:
        self.text_feature_count_ = int(text_bundle.text.shape[1])
        self.tree_feature_count_ = int(tree.shape[1])
        return OOFStackingFeatureBundle(
    ...
    def transform(self, features: pd.DataFrame) -> OOFStackingFeatureBundle:
        return self._bundle(
