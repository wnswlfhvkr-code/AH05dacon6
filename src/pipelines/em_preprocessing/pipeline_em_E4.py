"""train에서 반복되는 유전자 co-mutation pair 피처를 EMV16과 결합한 독립 실험 파이프라인입니다."""
...
class EME4PreprocessingPipeline(PreprocessingPipeline):
    """train에서 반복되는 유전자 co-mutation pair 피처; 다른 버전 파이프라인을 상속하지 않습니다."""

    name = "em_E4"
    evaluation_folds = 5
    feature_spec = EMExperimentSpec(
        name="E4",
        description="train에서 반복되는 유전자 co-mutation pair 피처",
        comutation_pairs=True,
    )
