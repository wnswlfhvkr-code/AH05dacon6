...
class _F02DerivedFeaturePipeline(PreprocessingPipeline):
    """전체 변이 이벤트 수만 생성하는 독립 실험 파이프라인입니다."""
    ...
    def __init__(self, **_: object) -> None:
        ...
        self.steps = ("결측·WT 정규화", "셀 내 중복 토큰 제거", "전체 변이 이벤트 수")

    def _build_features(self, features: pd.DataFrame) -> pd.DataFrame:
        counts = _event_counts(features, self.gene_columns)
        return pd.DataFrame({
            "mutation_event_count": counts.sum(axis=1).astype("float32"),
        }, index=features.index)

    def fit(self, features: pd.DataFrame, labels: pd.Series) -> "EMF02PreprocessingPipeline":
...
class EMF02PreprocessingPipeline(PreprocessingPipeline):
    """EMV45를 베이스로 F02 전용 파생 피처를 결합합니다."""
    ...
    def __init__(self, **parameters: object) -> None:
        ...
        self.derived_pipeline = _F02DerivedFeaturePipeline(**parameters)
    ...
    def _combine(v45: pd.DataFrame, derived: pd.DataFrame) -> pd.DataFrame:
        ...
        prefixed.columns = [f"F02__{column}" for column in prefixed.columns]
        return pd.concat([v45, prefixed], axis=1)

    def fit(
        self, features: pd.DataFrame, labels: pd.Series
    ) -> "EMF02PreprocessingPipeline":
    ...
    def summary(self) -> dict[str, int]:
        ...
        result.update({
            ...
            "f02_derived_features": self.derived_feature_count_,
