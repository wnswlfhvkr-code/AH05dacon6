"""EM F17: 표본과 암종 signature의 cosine 유사도; label 통계는 inner-fold OOF로 생성합니다."""
...
class _F17DerivedFeaturePipeline(PreprocessingPipeline):
    """표본과 암종 signature의 cosine 유사도를 train 통계와 inner-fold OOF로 생성합니다."""
    ...
    def __init__(
        ...
        **_: object,
        ...
        self.steps = (
            ...
            "학습 행 inner-fold OOF 표본과 암종 signature의 cosine 유사도",
    ...
    def fit(self, features: pd.DataFrame, labels: pd.Series) -> "EMF17PreprocessingPipeline":
...
class EMF17PreprocessingPipeline(PreprocessingPipeline):
    """EMV45를 베이스로 F17 전용 파생 피처를 결합합니다."""
    ...
    def __init__(self, **parameters: object) -> None:
        ...
        self.derived_pipeline = _F17DerivedFeaturePipeline(**parameters)
    ...
    def _combine(v45: pd.DataFrame, derived: pd.DataFrame) -> pd.DataFrame:
        ...
        prefixed.columns = [f"F17__{column}" for column in prefixed.columns]
        return pd.concat([v45, prefixed], axis=1)

    def fit(
        self, features: pd.DataFrame, labels: pd.Series
    ) -> "EMF17PreprocessingPipeline":
    ...
    def summary(self) -> dict[str, int]:
        ...
        result.update({
            ...
            "f17_derived_features": self.derived_feature_count_,
