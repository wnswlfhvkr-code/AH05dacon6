...
def _matches(token: str) -> bool:
    return bool(_FRAMESHIFT.search(token) or _NONSENSE.search(token))


class _F13DerivedFeaturePipeline(PreprocessingPipeline):
    """fold-train 지지도를 통과한 유전자별 truncating 변이 여부를 생성합니다."""
    ...
    def __init__(self, min_feature_support: int = 2, **_: object) -> None:
        ...
        self.steps = ("결측·WT 정규화", "consequence 판정", "유전자별 truncating 여부")

    def _matrix(self, features: pd.DataFrame, columns: list[str]) -> pd.DataFrame:
        missing = set(columns) - set(features.columns)
        if missing:
            raise ValueError(f"누락된 유전자 컬럼이 있습니다: {sorted(missing)}")
        return pd.DataFrame({
            gene: features[gene].map(_tokens).map(
                lambda tokens: any(_matches(token) for token in tokens)
            ).astype("int8")
            for gene in columns
        }, index=features.index)

    def _build_features(self, features: pd.DataFrame) -> pd.DataFrame:
        matrix = self._matrix(features, self.selected_genes_)
        matrix.columns = [f"gene_truncating__{gene}" for gene in self.selected_genes_]
        return matrix

    def fit(self, features: pd.DataFrame, labels: pd.Series) -> "EMF13PreprocessingPipeline":
        self.gene_columns = features.columns.tolist()
        initial = self._matrix(features, self.gene_columns)
        ...
        if not self.selected_genes_:
            raise ValueError("truncating 지지도를 만족하는 유전자가 없습니다.")
    ...
    def summary(self) -> dict[str, int]:
        ...
        result.update({
            "truncating_gene_features": len(self.selected_genes_),
            "dropped_gene_features": len(self.gene_columns) - len(self.selected_genes_),
        })
        return result


class EMF13PreprocessingPipeline(PreprocessingPipeline):
    """EMV45를 베이스로 F13 전용 파생 피처를 결합합니다."""
    ...
    def __init__(self, **parameters: object) -> None:
        ...
        self.derived_pipeline = _F13DerivedFeaturePipeline(**parameters)
    ...
    def _combine(v45: pd.DataFrame, derived: pd.DataFrame) -> pd.DataFrame:
        ...
        prefixed.columns = [f"F13__{column}" for column in prefixed.columns]
        return pd.concat([v45, prefixed], axis=1)

    def fit(
        self, features: pd.DataFrame, labels: pd.Series
    ) -> "EMF13PreprocessingPipeline":
    ...
    def summary(self) -> dict[str, int]:
        ...
        result.update({
            ...
            "f13_derived_features": self.derived_feature_count_,
