...
class _F09DerivedFeaturePipeline(PreprocessingPipeline):
    """유전자별 변이 여부를 fold-train 지지도 필터 후 생성합니다."""
    ...
    def __init__(self, min_feature_support: int = 2, **_: object) -> None:
        ...
        self.steps = ("결측·WT 정규화", "fold-train 지지도 필터", "유전자별 변이 여부")
    ...
    def fit(self, features: pd.DataFrame, labels: pd.Series) -> "EMF09PreprocessingPipeline":
        ...
        if not self.selected_genes_:
            raise ValueError("유전자별 변이 여부 지지도를 만족하는 유전자가 없습니다.")
...
class EMF09PreprocessingPipeline(PreprocessingPipeline):
    """EMV45를 베이스로 F09 전용 파생 피처를 결합합니다."""
    ...
    def __init__(self, **parameters: object) -> None:
        ...
        self.derived_pipeline = _F09DerivedFeaturePipeline(**parameters)
    ...
    def _combine(v45: pd.DataFrame, derived: pd.DataFrame) -> pd.DataFrame:
        ...
        prefixed.columns = [f"F09__{column}" for column in prefixed.columns]
        return pd.concat([v45, prefixed], axis=1)

    def fit(
        self, features: pd.DataFrame, labels: pd.Series
    ) -> "EMF09PreprocessingPipeline":
    ...
    def summary(self) -> dict[str, int]:
        ...
        result.update({
            ...
            "f09_derived_features": self.derived_feature_count_,
