"""EM F18: 상위 signature와 두 번째 signature의 차이; label 통계는 inner-fold OOF로 생성합니다."""
...
class _F18DerivedFeaturePipeline(PreprocessingPipeline):
    """상위 signature와 두 번째 signature의 차이를 train 통계와 inner-fold OOF로 생성합니다."""
    ...
    def __init__(
        ...
        **_: object,
        ...
        self.steps = (
            ...
            "학습 행 inner-fold OOF 상위 signature와 두 번째 signature의 차이",
    ...
    def fit(self, features: pd.DataFrame, labels: pd.Series) -> "EMF18PreprocessingPipeline":
...
class EMF18PreprocessingPipeline(PreprocessingPipeline):
    """EMV45를 베이스로 F18 전용 파생 피처를 결합합니다."""
    ...
    def __init__(self, **parameters: object) -> None:
        ...
        self.derived_pipeline = _F18DerivedFeaturePipeline(**parameters)
    ...
    def _combine(v45: pd.DataFrame, derived: pd.DataFrame) -> pd.DataFrame:
        ...
        prefixed.columns = [f"F18__{column}" for column in prefixed.columns]
        return pd.concat([v45, prefixed], axis=1)

    def fit(
        self, features: pd.DataFrame, labels: pd.Series
    ) -> "EMF18PreprocessingPipeline":
    ...
    def summary(self) -> dict[str, int]:
        ...
        result.update({
            ...
            "f18_derived_features": self.derived_feature_count_,
