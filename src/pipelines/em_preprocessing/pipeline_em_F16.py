"""EM F16: 암종별 weighted mutation score; label 통계는 inner-fold OOF로 생성합니다."""
...
class _F16DerivedFeaturePipeline(PreprocessingPipeline):
    """암종별 weighted mutation score를 train 통계와 inner-fold OOF로 생성합니다."""
    ...
    def __init__(
        ...
        **_: object,
        ...
        self.steps = (
            ...
            "학습 행 inner-fold OOF 암종별 weighted mutation score",
    ...
    def fit(self, features: pd.DataFrame, labels: pd.Series) -> "EMF16PreprocessingPipeline":
...
class EMF16PreprocessingPipeline(PreprocessingPipeline):
    """EMV45를 베이스로 F16 전용 파생 피처를 결합합니다."""
    ...
    def __init__(self, **parameters: object) -> None:
        ...
        self.derived_pipeline = _F16DerivedFeaturePipeline(**parameters)
    ...
    def _combine(v45: pd.DataFrame, derived: pd.DataFrame) -> pd.DataFrame:
        ...
        prefixed.columns = [f"F16__{column}" for column in prefixed.columns]
        return pd.concat([v45, prefixed], axis=1)

    def fit(
        self, features: pd.DataFrame, labels: pd.Series
    ) -> "EMF16PreprocessingPipeline":
    ...
    def summary(self) -> dict[str, int]:
        ...
        result.update({
            ...
            "f16_derived_features": self.derived_feature_count_,
