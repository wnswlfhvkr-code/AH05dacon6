"""EM F04: consequence별 표본 변이 이벤트 수."""
...
def _kind(token: str) -> str | None:
    if _SPLICE.search(token):
        return "splice"
    if _FRAMESHIFT.search(token):
        return "frameshift"
    if _NONSENSE.search(token):
        return "nonsense"
...
class _F04DerivedFeaturePipeline(PreprocessingPipeline):
    """missense·nonsense·frameshift·splice 이벤트 수를 생성합니다."""
    ...
    def __init__(self, **_: object) -> None:
        ...
        self.steps = ("결측·WT 정규화", "중복 토큰 제거", "consequence별 이벤트 수")
    ...
    def _build_features(self, features: pd.DataFrame) -> pd.DataFrame:
        ...
        output = pd.DataFrame(0, index=features.index, columns=[
            "missense_event_count", "nonsense_event_count",
            "frameshift_event_count", "splice_event_count",
        ], dtype="int32")
        for column in self.gene_columns:
            for row_index, value in features[column].items():
                for token in _tokens(value):
                    kind = _kind(token)
                    if kind is not None:
                        output.at[row_index, f"{kind}_event_count"] += 1
        return output

    def fit(self, features: pd.DataFrame, labels: pd.Series) -> "EMF04PreprocessingPipeline":
    ...
    def summary(self) -> dict[str, int]:
        ...
        result["derived_feature_count"] = 4
        return result


class EMF04PreprocessingPipeline(PreprocessingPipeline):
    """EMV45를 베이스로 F04 전용 파생 피처를 결합합니다."""
    ...
    def __init__(self, **parameters: object) -> None:
        ...
        self.derived_pipeline = _F04DerivedFeaturePipeline(**parameters)
    ...
    def _combine(v45: pd.DataFrame, derived: pd.DataFrame) -> pd.DataFrame:
        ...
        prefixed.columns = [f"F04__{column}" for column in prefixed.columns]
        return pd.concat([v45, prefixed], axis=1)

    def fit(
        self, features: pd.DataFrame, labels: pd.Series
    ) -> "EMF04PreprocessingPipeline":
    ...
    def summary(self) -> dict[str, int]:
        ...
        result.update({
            ...
            "f04_derived_features": self.derived_feature_count_,
