"""변이 부담(v5), 기능 결과(v6), 암종 signature(v9)를 결합한 EM 버전 13입니다."""
...
def create_mutation_presence_matrix(features: pd.DataFrame) -> pd.DataFrame:
    """v13의 최소 빈도 계산을 위한 WT/변이 이진 행렬입니다."""
    return pd.DataFrame(
        {
...
def select_genes_by_mutation_count(
    ...
    """v13 학습 데이터에서 최소 변이 횟수를 만족하는 유전자를 선택합니다."""
    mutation_counts = (
...
def create_consequence_severity_matrix(
    ...
    """v13 유전자 변이를 WT=0부터 truncating=4까지로 인코딩합니다."""
...
def add_mutation_burden_features(
    ...
    """v13의 변이 부담 total/log1p/rate를 한 번만 추가합니다."""
...
def learn_class_gene_weights(
    ...
    """v13 학습 fold에서 암종별 안정화 log2 odds 가중치를 학습합니다."""
...
def add_class_signature_features(
    ...
    """v13의 암종별 weighted/match signature를 추가합니다."""
    for class_name in class_names:
...
class EMV13PreprocessingPipeline(PreprocessingPipeline):
    """severity가 이진 변이를 대체하고 burden·signature 중복 열은 한 번만 둡니다."""
    ...
    def __init__(
        ...
        **parameters: object,
        ...
        self.steps = ("최소 변이 빈도 필터", "기능 결과 심각도", "변이 부담", "기능 결과 구성", "암종 signature")
    ...
    def fit(self, features: pd.DataFrame, labels: pd.Series) -> "EMV13PreprocessingPipeline":
    ...
    def summary(self) -> dict[str, int]:
        ...
        summary.update({"dropped_rare_features": len(self.dropped_rare_columns), "burden_features": 3, "consequence_features": 8, "signature_features": 2 * len(self.class_names)})
        return summary
