"""EMV24와 E14(E7·E3·E6)를 중복 없이 결합한 독립 EMV45."""
...
def create_oof_pattern_uncertainty(
    ...
    for train_positions, valid_positions in splitter.split(features, labels):
        ...
        fold_output = create_pattern_uncertainty_features(
...
def add_summary_features(
    ...
    output["token_excess_count_log1p"] = np.log1p(
        np.maximum(token_count - mutated_gene_count, 0.0)
    ).astype("float32")
    output["max_gene_token_count"] = max_gene_token_counts.astype("float32")
    output["multi_hit_gene_ratio"] = (
...
def add_dual_pair_contrasts(output: pd.DataFrame) -> None:
    ...
    for channel in ("all", "functional"):
        for left, right in (("KIRC", "KIPAN"), ("LGG", "GBMLGG")):
            ...
            if left_weighted in output and right_weighted in output:
                output[f"pair_{channel}_{left}_{right}_weighted_diff"] = (
                    output[left_weighted] - output[right_weighted]
                )
                output[f"pair_{channel}_{left}_{right}_count_diff"] = (
...
class EMV45PreprocessingPipeline(PreprocessingPipeline):
    ...
    def _build_features(self, features: pd.DataFrame) -> pd.DataFrame:
        ...
        add_dual_pair_contrasts(output)
        return pd.concat(
            [
                output,
    ...
    def fit(self, features: pd.DataFrame, labels: pd.Series) -> "EMV45PreprocessingPipeline":
    ...
    def fit_transform(self, features: pd.DataFrame, labels: pd.Series) -> pd.DataFrame:
        ...
        add_dual_pair_contrasts(transformed)
