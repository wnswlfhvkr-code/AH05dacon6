"""v01의 F0·F1·F3 변이 피처 생성 파이프라인입니다."""
...
def split_mutation_tokens(value: object) -> list[str]:
    ...
    return list(dict.fromkeys(
        ...
        if token.strip() and normalize_event_token(token) != "WT"
    ))
...
def parse_wide_mutations(features: pd.DataFrame) -> pd.DataFrame:
    ...
    for gene in features.columns:
        ...
        candidate_positions = np.flatnonzero(
            (normalized.notna() & normalized.ne("") & normalized.ne("WT")).to_numpy()
        )
...
def build_f0_matrix(features: pd.DataFrame) -> sparse.csr_matrix:
    ...
    for gene in features.columns:
        ...
        columns.append((normalized.notna() & normalized.ne("") & normalized.ne("WT")).to_numpy(dtype=np.float32))
...
class JHV01PreprocessingPipeline:
    ...
    def fit(self, features: pd.DataFrame, labels: pd.Series) -> "JHV01PreprocessingPipeline":
...
JHV1PreprocessingPipeline = JHV01PreprocessingPipeline
