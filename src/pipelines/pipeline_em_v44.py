"""EMV16 핵심과 E14의 E7·E3·E6 기능을 하나로 합친 독립 EMV44 파이프라인입니다.

결측·WT 처리, 저빈도 유전자 필터, consequence·burden, inner-fold OOF
class signature, recurrent hotspot, OOF 패턴 불확실성, multi-hit token 복잡도,
중첩 암종 pair contrast를 이 파일 안에서 모두 처리합니다.
"""
...
class EMV44PreprocessingPipeline(PreprocessingPipeline):
    """EMV16 + E7 OOF 불확실성 + E3 multi-hit + E6 pair contrast입니다."""
    ...
    feature_spec = EMExperimentSpec(
        name="V44",
        description=(
            "EMV16 + E7 OOF 패턴 불확실성 + E3 multi-hit token complexity + "
