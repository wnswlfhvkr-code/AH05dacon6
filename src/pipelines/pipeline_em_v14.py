"""기능 결과·암종 signature에 반복 hotspot을 추가한 EM 버전 14입니다."""
...
def select_genes_by_mutation_count(
    ...
    """v14 학습 데이터에서 최소 변이 횟수를 만족하는 유전자를 선택합니다."""
    mutation_counts = (
...
def create_consequence_severity_matrix(
    ...
    """v14 유전자 변이를 WT=0부터 truncating=4까지로 인코딩합니다."""
    missing_columns = set(columns) - set(features.columns)
    if missing_columns:
...
def add_consequence_summary_features(
    ...
    """v14의 결과 유형별 개수와 비율 및 변이 부담을 추가합니다."""
...
def learn_class_gene_weights(
    ...
    """v14 학습 Fold에서 암종별 안정화 log2 odds 가중치를 학습합니다."""
...
def add_class_signature_features(
    ...
    """v14의 암종별 weighted/match signature를 추가합니다."""
    for class_name in class_names:
...
class EMV14PreprocessingPipeline(PreprocessingPipeline):
    ...
    def fit(self, features: pd.DataFrame, labels: pd.Series) -> "EMV14PreprocessingPipeline":
