...
class F3PreprocessingPipeline(PreprocessingPipeline):
    """F3PreprocessingPipeline의 단계 고정 전처리 구현입니다."""
    name = 'jyp_f3'
    ...
    def __init__(self, *, burden_clip_quantile: float=0.99, f3_position_min_support: int=2, f3_aa_min_support: int=2, show_progress: bool=True, progress_interval: int=25000) -> None:
        ...
        self.feature_blocks = ('raw', 'f0', 'f1', 'f2', 'f3_position', 'f3_aa')
    ...
    def fit(self, features: pd.DataFrame, labels: pd.Series | Sequence[str]) -> 'F3PreprocessingPipeline':
