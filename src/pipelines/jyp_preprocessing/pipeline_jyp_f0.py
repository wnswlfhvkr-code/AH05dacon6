...
class F0PreprocessingPipeline(PreprocessingPipeline):
    """F0PreprocessingPipeline의 단계 고정 전처리 구현입니다."""
    name = 'jyp_f0'
    ...
    def fit(self, features: pd.DataFrame, labels: pd.Series | Sequence[str]) -> 'F0PreprocessingPipeline':
        ...
        self._log(f'fit 완료 | pipeline={self.pipeline_name}, RAW={len(self.gene_columns_):,}, F0={len(self.active_gene_columns_):,}, F1={0}, F2={len(self.f2_feature_names_):,}, F3_POS={len(self.f3_position_feature_names_):,}, F3_AA={len(self.f3_aa_feature_names_):,}, F4={len(self.f4_feature_names_):,}, F5={len(self.f5_feature_names_):,}, F7={len(self.f7_feature_names_):,}')
        return self
