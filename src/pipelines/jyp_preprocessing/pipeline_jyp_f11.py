"""jyp_f11 고정 JYP 전처리 파이프라인입니다.

AGENTS.md의 버전 독립 규칙에 따라 이 파일이 사용하는 파싱, 피처 생성,
학습 및 변환 로직을 파일 안에 직접 보관합니다.

F9의 피처 순서를 그대로 유지하고, 학습 파티션 내부 OOF 혼동행렬에서
자동 선택한 비수동 class pair의 contrast 블록을 마지막에 붙입니다.
pair 스키마는 학습 파티션 전체의 감독형 OOF 혼동으로 한 번 선택하고,
각 학습 행의 contrast 값만 내부 OOF로 계산합니다. 따라서 외부 검증·테스트
경계는 안전하지만 pair 스키마까지 행 단위로 완전 cross-fit된 것은 아닙니다.
"""
...
@dataclass(frozen=True)
class F11ConfusionPairCandidate:
    """OOF surrogate 혼동행렬에서 계산한 하나의 class pair 후보입니다."""
    rank: int
    pair: F7OrderedPair
    confusion_rate: float
    swapped_errors: int
    combined_support: int
    excluded_manual: bool
...
def build_f11_feature_names(ordered_pairs: Sequence[F7OrderedPair | Sequence[object]]) -> list[str]:
    """자동 혼동쌍마다 기존 F7과 같은 두 점수 이름을 고정 순서로 만듭니다."""
    pairs = tuple((_normalize_pair(pair) for pair in ordered_pairs))
    return [
        f"F11__auto_confusion__{pair.first_label.replace(' ', '_')}_vs_{pair.second_label.replace(' ', '_')}__{score_name}"
        for pair in pairs
        for score_name in F7_SCORE_NAMES
    ]

def discover_f11_confusion_pairs(
    matrix: sparse.spmatrix,
    labels: Sequence[object] | np.ndarray,
    excluded_pairs: Sequence[F7OrderedPair | Sequence[object]],
    *,
    pair_count: int=6,
    discovery_folds: int=3,
    regularization_c: float=0.07,
    max_iter: int=500,
    random_state: int=42,
) -> tuple[
    tuple[F11ConfusionPairCandidate, ...],
    tuple[str, ...],
    np.ndarray,
    int,
]:
    """학습 파티션 내부 Logistic OOF 혼동으로 비수동 pair를 선택합니다."""
    for value, name, minimum in (
        (pair_count, 'pair_count', 1),
        (discovery_folds, 'discovery_folds', 2),
        (max_iter, 'max_iter', 1),
        (random_state, 'random_state', 0),
    ):
        if isinstance(value, bool) or not isinstance(value, int) or value < minimum:
            raise ValueError(f'F11 {name}은 {minimum} 이상의 정수여야 합니다.')
    if not np.isfinite(regularization_c) or regularization_c <= 0:
        raise ValueError('F11 regularization_c는 유한한 양수여야 합니다.')
    mutations = sparse.csr_matrix(matrix, dtype=np.float32, copy=True)
    mutations.sum_duplicates()
    mutations.eliminate_zeros()
    if mutations.nnz:
        if not np.isfinite(mutations.data).all() or np.any(mutations.data < 0):
            raise ValueError('F11 discovery matrix는 유한한 비음수여야 합니다.')
        mutations.data.fill(1.0)
    normalized_labels = np.asarray([str(label) for label in labels], dtype=str)
    if normalized_labels.ndim != 1 or len(normalized_labels) != mutations.shape[0]:
        raise ValueError('F11 discovery matrix와 labels의 행 수가 일치해야 합니다.')
    classes, class_counts = np.unique(normalized_labels, return_counts=True)
    if len(classes) < 2:
        raise ValueError('F11 discovery에는 두 개 이상의 클래스가 필요합니다.')
    if class_counts.min() < discovery_folds:
        raise ValueError(
            f'F11 discovery는 클래스마다 최소 {discovery_folds}개 샘플이 필요합니다.'
        )
    normalized_exclusions = tuple((_normalize_pair(pair) for pair in excluded_pairs))
    excluded_keys = {
        frozenset((pair.first_label, pair.second_label))
        for pair in normalized_exclusions
    }
    available_pair_count = len(classes) * (len(classes) - 1) // 2 - sum(
        frozenset((classes[first], classes[second])) in excluded_keys
        for first in range(len(classes))
        for second in range(first + 1, len(classes))
    )
    if available_pair_count < pair_count:
        raise ValueError(
            f'F11 discovery 후보가 부족합니다: 필요 {pair_count}, 사용 가능 {available_pair_count}.'
        )
    predictions = np.empty(len(normalized_labels), dtype=object)
    convergence_warning_count = 0
    splitter = StratifiedKFold(
        n_splits=discovery_folds,
        shuffle=True,
        random_state=random_state,
    )
    for fold_number, (train_indices, valid_indices) in enumerate(
        splitter.split(np.zeros(len(normalized_labels)), normalized_labels),
        start=1,
    ):
        ...
        with warnings.catch_warnings(record=True) as caught:
            ...
            surrogate.fit(mutations[train_indices], normalized_labels[train_indices])
        convergence_warning_count += sum(
            issubclass(item.category, ConvergenceWarning) for item in caught
        )
        predictions[valid_indices] = surrogate.predict(mutations[valid_indices])
    if convergence_warning_count:
        raise RuntimeError(
            f'F11 discovery surrogate가 수렴하지 않았습니다: '
            f'{convergence_warning_count}개 fold 경고.'
        )
    ...
    ranked_values: list[tuple[float, int, str, str, int]] = []
    for first_index in range(len(classes)):
        for second_index in range(first_index + 1, len(classes)):
            swapped_errors = int(
                ...
                + confusion[second_index, first_index]
            )
            combined_support = int(supports[first_index] + supports[second_index])
            rate = swapped_errors / max(combined_support, 1)
            ranked_values.append(
                (
                    float(rate),
                    swapped_errors,
                    str(classes[first_index]),
                    str(classes[second_index]),
                    combined_support,
                )
            )
    ranked_values.sort(
    ...
    selected_count = 0
    catalog: list[F11ConfusionPairCandidate] = []
    for rank, (rate, swapped, first_label, second_label, support) in enumerate(
        ranked_values,
        start=1,
    ):
        pair = F7OrderedPair(first_label, second_label)
        excluded_manual = (
            frozenset((pair.first_label, pair.second_label)) in excluded_keys
        )
        selected_rank = None
        if not excluded_manual and selected_count < pair_count:
            selected_count += 1
            selected_rank = selected_count
        catalog.append(
            F11ConfusionPairCandidate(
                rank=rank,
                pair=pair,
                confusion_rate=rate,
                swapped_errors=swapped,
                combined_support=support,
                excluded_manual=excluded_manual,
                selected_rank=selected_rank,
            )
        )
    if selected_count != pair_count:
        raise RuntimeError('F11 discovery가 요청한 pair 수를 선택하지 못했습니다.')
    return (
        tuple(catalog),
...
def relabel_f11_pair_model(model: F7PairContrastModel) -> F7PairContrastModel:
    """F7 계산 계약을 유지하면서 자동 pair 출력 이름만 F11로 분리합니다."""
    return replace(
        model,
        feature_names=tuple(build_f11_feature_names(model.ordered_pairs)),
    )
...
def build_f7_selected_gene_catalog(model: F7PairContrastModel) -> list[dict[str, object]]:
    ...
    return catalog
class F11AutoConfusionPairNoRawPreprocessingPipeline(PreprocessingPipeline):
    """감독 선택한 pair 스키마에 OOF contrast 값을 채우는 F11 구현입니다."""
    name = 'jyp_f11'
    ...
    def __init__(
        ...
        *,
        ...
        for value, name, minimum in (
            (f11_auto_pair_count, 'f11_auto_pair_count', 1),
            (f11_discovery_folds, 'f11_discovery_folds', 2),
            (f11_discovery_max_iter, 'f11_discovery_max_iter', 1),
            (f11_random_state, 'f11_random_state', 0),
        ):
            if isinstance(value, bool) or not isinstance(value, int) or value < minimum:
                raise ValueError(f'{name}은 {minimum} 이상의 정수여야 합니다.')
        if not np.isfinite(f11_discovery_c) or f11_discovery_c <= 0:
            raise ValueError('f11_discovery_c는 유한한 양수여야 합니다.')
        self.f11_auto_pair_count = int(f11_auto_pair_count)
        self.f11_discovery_folds = int(f11_discovery_folds)
        self.f11_discovery_c = float(f11_discovery_c)
        self.f11_discovery_max_iter = int(f11_discovery_max_iter)
        self.f11_random_state = int(f11_random_state)

    def fit(self, features: pd.DataFrame, labels: pd.Series | Sequence[str]) -> 'F11AutoConfusionPairNoRawPreprocessingPipeline':
        ...
        missing_gene_matrix = build_missing_gene_matrix(frame)
        self.f7_pair_contrast_model_, self._fit_f7_oof_matrix_ = fit_with_oof(scan.f0_all_genes, missing_gene_matrix, label_values, self.gene_columns_, self.f7_pairs, oof_folds=self.f7_stability_folds, top_k_per_direction=self.f7_top_k_per_direction, minimum_pair_mutation_support=self.f7_min_gene_support, laplace_alpha=self.f7_laplace_alpha, burden_quantiles=self.f7_burden_quantiles, stability_folds=self.f7_stability_folds, minimum_direction_consistency=self.f7_min_direction_consistency, minimum_selection_frequency=self.f7_min_selection_frequency, random_state=self.f7_random_state)
        ...
        (
            self.f11_confusion_pair_candidates_,
            self.f11_discovery_labels_,
            self.f11_discovery_confusion_matrix_,
            self.f11_discovery_convergence_warning_count_,
        ) = discover_f11_confusion_pairs(
            scan.f0_all_genes[:, self.active_gene_indices_],
            label_values,
            self.f7_pairs,
            pair_count=self.f11_auto_pair_count,
            discovery_folds=self.f11_discovery_folds,
            regularization_c=self.f11_discovery_c,
            max_iter=self.f11_discovery_max_iter,
            random_state=self.f11_random_state,
        )
        selected_candidates = sorted(
            (
                candidate
                for candidate in self.f11_confusion_pair_candidates_
                if candidate.selected_rank is not None
            ),
            key=lambda candidate: int(candidate.selected_rank),
        )
        self.f11_pairs_ = tuple((candidate.pair for candidate in selected_candidates))
        raw_f11_model, self._fit_f11_oof_matrix_ = fit_with_oof(
            scan.f0_all_genes,
            missing_gene_matrix,
            label_values,
            self.gene_columns_,
            self.f11_pairs_,
            oof_folds=self.f7_stability_folds,
            top_k_per_direction=self.f7_top_k_per_direction,
            minimum_pair_mutation_support=self.f7_min_gene_support,
            laplace_alpha=self.f7_laplace_alpha,
            burden_quantiles=self.f7_burden_quantiles,
            stability_folds=self.f7_stability_folds,
            minimum_direction_consistency=self.f7_min_direction_consistency,
            minimum_selection_frequency=self.f7_min_selection_frequency,
            random_state=self.f11_random_state,
        )
        self.f11_pair_contrast_model_ = relabel_f11_pair_model(raw_f11_model)
        self.f11_feature_names_ = list(self.f11_pair_contrast_model_.feature_names)
        f11_selected_arrays = [
            np.asarray(statistics.selected_gene_indices, dtype=np.int64)
            for statistics in self.f11_pair_contrast_model_.pair_statistics
            if statistics.selected_gene_indices
        ]
        self.f11_selected_gene_indices_ = (
            np.unique(np.concatenate(f11_selected_arrays))
            if f11_selected_arrays
            else np.asarray([], dtype=np.int64)
        )
        self.f11_selected_gene_catalog_ = build_f7_selected_gene_catalog(
            self.f11_pair_contrast_model_
        )
        names: list[str] = []
    ...
    def _build_matrix(self, frame: pd.DataFrame, *, scan: MutationScanResult, use_fit_oof: bool) -> sparse.csr_matrix:
        ...
        if use_fit_oof:
            if self._fit_f11_oof_matrix_ is None:
                raise RuntimeError('F11 OOF 행렬이 준비되지 않았습니다.')
            if self._fit_f11_oof_matrix_.shape[0] != len(frame):
                raise RuntimeError('F11 OOF 행렬과 학습 입력의 행 수가 다릅니다.')
            blocks.append(self._fit_f11_oof_matrix_)
        else:
            blocks.append(
                build_f7_matrix(
                    scan.f0_all_genes,
                    build_missing_gene_matrix(frame),
                    self.f11_pair_contrast_model_,
                )
            )
    ...
    def fit_transform(self, features: pd.DataFrame, labels: pd.Series | Sequence[str]):
        ...
        self._log('F11 pair 스키마는 학습 전체의 OOF 혼동으로 감독 선택하며, contrast 값은 내부 OOF로 계산합니다. 외부 검증·테스트는 train-fit 스키마만 사용합니다.')
    ...
    def summary(self) -> dict[str, object]:
        ...
        return {
            ...
            'f11_discovery_model': 'logistic_regression_lbfgs_f0_binary',
            'f11_discovery_folds': self.f11_discovery_folds,
            'f11_discovery_c': self.f11_discovery_c,
            'f11_candidate_pair_count': len(self.f11_confusion_pair_candidates_),
            'f11_excluded_manual_pair_count': sum(candidate.excluded_manual for candidate in self.f11_confusion_pair_candidates_),
            'f11_selected_pair_count': len(self.f11_pairs_),
            'f11_selected_pairs': [[pair.first_label, pair.second_label] for pair in self.f11_pairs_],
            'f11_auto_pair_features': len(self.f11_feature_names_),
            'f11_selected_gene_union': len(self.f11_selected_gene_indices_),
            'f11_discovery_convergence_warnings': self.f11_discovery_convergence_warning_count_,
            'f11_training_value_semantics': 'pair_schema_supervised_full_training__values_oof__outer_validation_train_fitted',
    ...
    def get_diagnostics(self) -> dict[str, object]:
        ...
        return {
            ...
            'f11_confusion_pair_candidates': [
                {
                    'rank': candidate.rank,
                    'first_label': candidate.pair.first_label,
                    'second_label': candidate.pair.second_label,
                    'confusion_rate': candidate.confusion_rate,
                    'swapped_errors': candidate.swapped_errors,
                    'combined_support': candidate.combined_support,
                    'excluded_manual': candidate.excluded_manual,
                    'selected_rank': candidate.selected_rank,
                }
                for candidate in self.f11_confusion_pair_candidates_
            ],
            'f11_discovery_labels': list(self.f11_discovery_labels_),
            'f11_discovery_confusion_matrix': self.f11_discovery_confusion_matrix_.copy(),
            'f11_pair_contrast_model': self.f11_pair_contrast_model_,
            'f11_selected_gene_indices': self.f11_selected_gene_indices_.copy(),
            'f11_selected_gene_catalog': [
                dict(item) for item in self.f11_selected_gene_catalog_
            ],
            'f11_fit_oof_matrix': self._fit_f11_oof_matrix_,
        }
    ...
    def __getstate__(self) -> dict[str, object]:
        ...
        state['_fit_f11_oof_matrix_'] = None
        return state
