...
def build_f7_selected_gene_catalog(model: F7PairContrastModel) -> list[dict[str, object]]:
    ...
    return catalog
class F7PairContrastNoRawPreprocessingPipeline(PreprocessingPipeline):
    """F7PairContrastNoRawPreprocessingPipeline의 단계 고정 전처리 구현입니다."""
    name = 'jyp_f7'
    ...
    def __init__(
        ...
        *,
        ...
        self.feature_blocks = ('f0', 'f1', 'f2', 'f3_position', 'f3_aa', 'f4', 'f7_paircontrast')
    ...
    def fit(self, features: pd.DataFrame, labels: pd.Series | Sequence[str]) -> 'F7PairContrastNoRawPreprocessingPipeline':
        ...
        self._log(f'fit 완료 | pipeline={self.pipeline_name}, RAW={0:,}, F0={len(self.active_gene_columns_):,}, F1={len(F1_COLUMNS)}, F2={len(self.f2_feature_names_):,}, F3_POS={len(self.f3_position_feature_names_):,}, F3_AA={len(self.f3_aa_feature_names_):,}, F4={len(self.f4_feature_names_):,}, F7={len(self.f7_feature_names_):,}')
        return self
