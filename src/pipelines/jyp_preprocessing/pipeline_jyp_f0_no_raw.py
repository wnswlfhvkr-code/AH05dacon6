"""jyp_f0_no_raw 고정 JYP 전처리 파이프라인입니다.
...
class F0NoRawPreprocessingPipeline(PreprocessingPipeline):
    """F0NoRawPreprocessingPipeline의 단계 고정 전처리 구현입니다."""
    name = 'jyp_f0_no_raw'
    ...
    def fit(self, features: pd.DataFrame, labels: pd.Series | Sequence[str]) -> 'F0NoRawPreprocessingPipeline':
        ...
        self._log(f'fit 완료 | pipeline={self.pipeline_name}, RAW={0:,}, F0={len(self.active_gene_columns_):,}, F1={0}, F2={len(self.f2_feature_names_):,}, F3_POS={len(self.f3_position_feature_names_):,}, F3_AA={len(self.f3_aa_feature_names_):,}, F4={len(self.f4_feature_names_):,}, F5={len(self.f5_feature_names_):,}, F7={len(self.f7_feature_names_):,}')
        return self
