"""jyp_f4_no_raw 고정 JYP 전처리 파이프라인입니다.
...
class F4NoRawPreprocessingPipeline(PreprocessingPipeline):
    """F4NoRawPreprocessingPipeline의 단계 고정 전처리 구현입니다."""
    name = 'jyp_f4_no_raw'
    ...
    def fit(self, features: pd.DataFrame, labels: pd.Series | Sequence[str]) -> 'F4NoRawPreprocessingPipeline':
        ...
        self._log(f'fit 완료 | pipeline={self.pipeline_name}, RAW={0:,}, F0={len(self.active_gene_columns_):,}, F1={len(F1_COLUMNS)}, F2={len(self.f2_feature_names_):,}, F3_POS={len(self.f3_position_feature_names_):,}, F3_AA={len(self.f3_aa_feature_names_):,}, F4={len(self.f4_feature_names_):,}')
        return self

    def transform(self, features: pd.DataFrame):
        self._require_fitted()
        frame = self._validate_transform_frame(features)
        reuse_fit_scan = getattr(self, '_fit_frame_reference_', None) is features
        scan = self._fit_scan_ if reuse_fit_scan else self._scan(frame, stage='transform')
        return self._build_matrix(scan)

    def _build_matrix(self, scan: MutationScanResult) -> sparse.csr_matrix:
        """학습 때 고정한 F0~F4 스키마로 희소 행렬을 생성합니다."""
    ...
    def fit_transform(self, features: pd.DataFrame, labels: pd.Series | Sequence[str]):
        ...
        self._validate_transform_frame(features)
        return self._build_matrix(self._fit_scan_)
    ...
    def summary(self) -> dict[str, object]:
        ...
        return {
            ...
            'f7_pair_count': 0,
            'f7_pair_contrast_features': 0,
            'f7_selected_gene_union': 0,
    ...
    def get_diagnostics(self) -> dict[str, object]:
        ...
        return {
            ...
            'f7_pair_contrast_model': None,
            'f7_selected_gene_indices': np.asarray([], dtype=np.int64),
            'f7_selected_gene_catalog': [],
            'f7_fit_oof_matrix': None,
        }
