"""jyp_f5_no_raw 고정 JYP 전처리 파이프라인입니다.
...
class F5NoRawPreprocessingPipeline(PreprocessingPipeline):
    ...
    def __init__(self, *, burden_clip_quantile: float=0.99, f3_position_min_support: int=2, f3_aa_min_support: int=2, f4_min_support: int=10, f5_top_k_per_class: int=25, f5_min_gene_support: int=5, f5_laplace_alpha: float=1.0, f5_stability_folds: int=5, f5_min_direction_consistency: int=4, f5_min_selection_frequency: int=3, f5_random_state: int=42, show_progress: bool=True, progress_interval: int=25000) -> None:
        ...
        normalized_output_scores = None
        normalized_output_class_labels = None
        ...
        self.f5_output_rare_class_count = None
        self.f5_output_score_names = normalized_output_scores
        self.f5_output_class_labels = normalized_output_class_labels

    def fit(self, features: pd.DataFrame, labels: pd.Series | Sequence[str]) -> 'F5NoRawPreprocessingPipeline':
        ...
        self._fit_f5_oof_matrix_ = full_f5_oof_matrix
        self.f5_feature_names_ = full_f5_feature_names
        selected_arrays = [indices for class_index, indices in enumerate(self.f5_signature_model_.selected_gene_indices) if True if len(indices)]
    ...
    def _build_matrix(self, frame: pd.DataFrame, *, scan: MutationScanResult | None, use_fit_oof: bool) -> sparse.csr_matrix:
        ...
        if use_fit_oof:
            ...
            f5_matrix = build_f5_signature_matrix(scan.f0_all_genes, self.f5_signature_model_, missing_matrix=None)
            blocks.append(f5_matrix)
    ...
    def _configure_f5_output(self, encoded_labels: np.ndarray) -> None:
        ...
        selected_score_names = F5_SCORE_NAMES
