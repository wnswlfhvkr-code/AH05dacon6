...
@dataclass
class F6CompressionModel:
    """Fold-Train에서 학습해 validation/test에 그대로 적용하는 F6 상태입니다."""
    minimum_document_frequency: int
    selected_token_indices: np.ndarray
    selected_token_feature_names: tuple[str, ...]
    selected_token_document_frequencies: np.ndarray
    requested_components: int
    actual_components: int
    explained_variance_ratio_sum: float
    tfidf_transformer: TfidfTransformer | None
    svd_transformer: TruncatedSVD | None
    f1_scaler: StandardScaler
    feature_names: tuple[str, ...]

def fit_f6_compression(binary_tokens: sparse.spmatrix, binary_token_feature_names: Sequence[str], f1_matrix: sparse.spmatrix, f1_feature_names: Sequence[str], *, minimum_document_frequency: int=2, maximum_components: int=128, random_state: int=42) -> F6CompressionModel:
    """DF 필터, TF-IDF, SVD와 F1 scaler를 Fold-Train에서 학습합니다."""
    if minimum_document_frequency < 1:
        raise ValueError('F6 minimum document frequency는 1 이상이어야 합니다.')
    if maximum_components < 1:
        raise ValueError('F6 maximum components는 1 이상이어야 합니다.')
    tokens = sparse.csr_matrix(binary_tokens, dtype=np.float32)
    f1 = sparse.csr_matrix(f1_matrix, dtype=np.float32)
    if tokens.shape[0] != f1.shape[0]:
        raise ValueError('F6 binary token과 F1 행 수가 일치하지 않습니다.')
    if tokens.shape[1] != len(binary_token_feature_names):
        raise ValueError('F6 binary token 이름 수가 열 수와 일치하지 않습니다.')
    if f1.shape[1] != len(f1_feature_names):
        raise ValueError('F6 F1 이름 수가 열 수와 일치하지 않습니다.')
    document_frequencies = np.diff(tokens.tocsc().indptr)
    selected_indices = np.flatnonzero(document_frequencies >= minimum_document_frequency).astype(np.int64, copy=False)
    selected_names = tuple((binary_token_feature_names[index] for index in selected_indices))
    selected_frequencies = document_frequencies[selected_indices].astype(np.int64, copy=False)
    ...
    if len(selected_indices):
        ...
        train_tfidf = tfidf.fit_transform(tokens[:, selected_indices])
        if train_tfidf.shape[1] >= 2 and train_tfidf.shape[0] >= 1:
            actual_components = min(maximum_components, train_tfidf.shape[0], train_tfidf.shape[1])
            svd = TruncatedSVD(n_components=actual_components, random_state=random_state)
            svd.fit(train_tfidf)
            explained_variance = float(np.nan_to_num(svd.explained_variance_ratio_, nan=0.0, posinf=0.0, neginf=0.0).sum())
    scaler = StandardScaler()
    scaler.fit(f1.toarray())
    feature_names = tuple([f'F6__f1_scaled__{name}' for name in f1_feature_names] + [f'F6__svd__component_{index + 1:03d}' for index in range(actual_components)])
    return F6CompressionModel(minimum_document_frequency=minimum_document_frequency, selected_token_indices=selected_indices, selected_token_feature_names=selected_names, selected_token_document_frequencies=selected_frequencies, requested_components=maximum_components, actual_components=actual_components, explained_variance_ratio_sum=explained_variance, tfidf_transformer=tfidf, svd_transformer=svd, f1_scaler=scaler, feature_names=feature_names)

def build_f6_matrix(binary_tokens: sparse.spmatrix, f1_matrix: sparse.spmatrix, model: F6CompressionModel) -> sparse.csr_matrix:
    """학습된 F6 schema로 F1 + compressed token matrix를 변환합니다."""
    tokens = sparse.csr_matrix(binary_tokens, dtype=np.float32)
    f1 = sparse.csr_matrix(f1_matrix, dtype=np.float32)
    if tokens.shape[0] != f1.shape[0]:
        raise ValueError('F6 binary token과 F1 행 수가 일치하지 않습니다.')
    scaled_f1 = model.f1_scaler.transform(f1.toarray()).astype(np.float32, copy=False)
    blocks: list[sparse.spmatrix] = [sparse.csr_matrix(scaled_f1)]
    if model.actual_components:
        if model.tfidf_transformer is None or model.svd_transformer is None:
            raise RuntimeError('F6 TF-IDF/SVD 학습 상태가 없습니다.')
        selected = tokens[:, model.selected_token_indices]
        tfidf = model.tfidf_transformer.transform(selected)
        compressed = model.svd_transformer.transform(tfidf).astype(np.float32, copy=False)
        blocks.append(sparse.csr_matrix(compressed))
    output = sparse.hstack(blocks, format='csr', dtype=np.float32)
    if output.shape[1] != len(model.feature_names):
        raise RuntimeError('F6 생성 피처 수와 고정 스키마가 일치하지 않습니다.')
    if output.nnz and (not np.isfinite(output.data).all()):
        raise RuntimeError('F6 출력에 finite가 아닌 값이 있습니다.')
    return output

class F6NoRawPreprocessingPipeline(PreprocessingPipeline):
    """F6NoRawPreprocessingPipeline의 단계 고정 전처리 구현입니다."""
    name = 'jyp_f6'
    ...
    def __init__(
        ...
        *,
        ...
        if f6_min_document_frequency < 1 or f6_max_components < 1:
            raise ValueError('F6 minimum DF와 maximum components는 1 이상이어야 합니다.')
        self.f6_min_document_frequency = int(f6_min_document_frequency)
        self.f6_max_components = int(f6_max_components)
        self.f6_random_state = int(f6_random_state)

    def fit(self, features: pd.DataFrame, labels: pd.Series | Sequence[str]) -> 'F6NoRawPreprocessingPipeline':
        ...
        self.f6_compression_model_: F6CompressionModel | None = None
        self.f6_selected_token_indices_ = np.asarray([], dtype=np.int64)
        self.f6_selected_token_feature_names_: list[str] = []
        self.f6_requested_components_ = self.f6_max_components
        self.f6_actual_components_ = 0
        self.f6_explained_variance_ratio_sum_ = 0.0
        self.f6_feature_names_: list[str] = []
        ...
        f6_tokens, f6_token_names = self._build_f6_binary_tokens(scan)
        self.f6_compression_model_: F6CompressionModel | None = fit_f6_compression(f6_tokens, f6_token_names, build_f1_matrix(scan, burden_clip_value=self.burden_clip_value_), F1_COLUMNS, minimum_document_frequency=self.f6_min_document_frequency, maximum_components=self.f6_max_components, random_state=self.f6_random_state)
        self.f6_selected_token_indices_ = self.f6_compression_model_.selected_token_indices.copy()
        self.f6_selected_token_feature_names_ = list(self.f6_compression_model_.selected_token_feature_names)
        self.f6_requested_components_ = self.f6_compression_model_.requested_components
        self.f6_actual_components_ = self.f6_compression_model_.actual_components
        self.f6_explained_variance_ratio_sum_ = self.f6_compression_model_.explained_variance_ratio_sum
        self.f6_feature_names_ = list(self.f6_compression_model_.feature_names)
        self.feature_names_out_ = np.asarray(self.f6_feature_names_, dtype=object)
        self._fit_scan_ = scan
        self._log(f'fit 완료 | pipeline={self.pipeline_name}, F1_SCALED={len(F1_COLUMNS)}, TOKEN_MIN_DF={len(self.f6_compression_model_.selected_token_indices):,}, SVD={self.f6_compression_model_.actual_components:,}')
        return self
    ...
    def _build_matrix(self, frame: pd.DataFrame, *, scan: MutationScanResult | None, use_fit_oof: bool) -> sparse.csr_matrix:
        ...
        f6_tokens, _ = self._build_f6_binary_tokens(scan)
        output = build_f6_matrix(f6_tokens, build_f1_matrix(scan, burden_clip_value=self.burden_clip_value_), self.f6_compression_model_)
    ...
    def _build_f6_binary_tokens(self, scan: MutationScanResult) -> tuple[sparse.csr_matrix, list[str]]:
        """기존 Fold-Train F0/F2/F3/F4 schema를 F6 입력 순서로 결합합니다."""
        blocks = [scan.f0_all_genes[:, self.active_gene_indices_], scan.f2_all_gene_consequences[:, self.active_f2_indices_], scan.f3_all_gene_position_bins[:, self.active_f3_position_indices_], scan.f3_all_gene_aa_transitions[:, self.active_f3_aa_indices_], build_exact_hotspot_matrix(scan, self.f4_hotspot_vocabulary_)]
        names = [f'F0__gene__{gene}' for gene in self.active_gene_columns_] + self.f2_feature_names_ + self.f3_position_feature_names_ + self.f3_aa_feature_names_ + self.f4_feature_names_
        matrix = sparse.hstack(blocks, format='csr', dtype=np.float32)
        if matrix.shape[1] != len(names):
            raise RuntimeError('F6 binary token 수와 이름 수가 일치하지 않습니다.')
        return (matrix, names)
