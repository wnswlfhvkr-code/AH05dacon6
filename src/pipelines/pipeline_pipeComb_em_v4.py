...
class _EMV46CorrelationPruner:
    ...
    def __init__(
        ...
        correlation_max_features: int = 5000,
        ...
        **_: object,
        ...
        self.correlation_method = str(correlation_method).lower()
        self.correlation_threshold = float(correlation_threshold)
        self.correlation_max_features = int(correlation_max_features)
    ...
    @staticmethod
    def _interpretability(column: str) -> int:
        upper = str(column).upper()
        if "__" not in upper:
            return 6
        if "HOTSPOT" in upper or "CONSEQUENCE" in upper:
            return 5
        if any(token in upper for token in ("GENE_MUTATED", "MULTI_HIT", "MUTATION_COUNT")):
            return 5
        if upper.startswith(("F09__", "F10__", "F11__", "F12__", "F13__", "F14__", "F15__")):
            return 4
    ...
    def _priorities(self, frame: pd.DataFrame) -> dict[str, tuple[float, ...]]:
        support = frame.abs().gt(self.active_epsilon).sum(axis=0)
        variance = frame.var(axis=0, ddof=0)
        return {
            column: (
                float(self._interpretability(column)),
    ...
    def _drop_exact_duplicates(
        ...
        for candidates in buckets.values():
            ...
            for column in candidates:
                ...
                for group in groups:
                    if np.array_equal(values, frame[group[0]].to_numpy(copy=False)):
                        group.append(column)
                        break
                else:
            ...
            for group in groups:
                if len(group) > 1:
                    winner = max(group, key=priorities.__getitem__)
                    dropped.extend(column for column in group if column != winner)
        self.dropped_exact_duplicate_ = dropped
        return frame.drop(columns=dropped)

    def _drop_high_correlations(
        self,
        frame: pd.DataFrame,
        priorities: dict[str, tuple[float, ...]],
    ) -> pd.DataFrame:
        ...
        if len(candidates) < 2:
            return frame
        values = frame[candidates].to_numpy(dtype="float32", copy=True)
    ...
    def fit(self, frame: pd.DataFrame) -> "_EMV46CorrelationPruner":
        ...
        if numeric.empty:
            raise ValueError("상관 제거 후 유지할 EMV46 피처가 없습니다.")
        priorities = self._priorities(numeric)
        numeric = self._drop_exact_duplicates(numeric, priorities)
        numeric = self._drop_high_correlations(numeric, priorities)
        self.kept_columns_ = numeric.columns.tolist()
        return self

    def transform(self, frame: pd.DataFrame) -> pd.DataFrame:
        if not self.kept_columns_:
            raise RuntimeError("상관 제거기를 fit한 뒤 transform해야 합니다.")
        missing = set(self.kept_columns_) - set(frame.columns)
        if missing:
            raise ValueError(f"상관 제거에 필요한 피처가 없습니다: {sorted(missing)}")
        return self._numeric(frame[self.kept_columns_])
...
class PipeCombEMV4PreprocessingPipeline(PreprocessingPipeline):
    ...
    def fit_transform(
        ...
        self.feature_columns = features.columns.tolist()
        emv46 = self.correlation_pruner.fit_transform(
            self.emv46_pipeline.fit_transform(features, labels)
        )
        text = self.jsj9_text_engine.fit_transform(features)
    ...
    def summary(self) -> dict[str, object]:
        ...
        result.update(self.correlation_pruner.summary())
        return result
