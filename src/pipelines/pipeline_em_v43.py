"""em_v24와 em_v15를 중복 없이 결합한 EM v43."""
...
def learn_synonymous_hotspots(
    features: pd.DataFrame,
    columns: list[str],
    minimum_count: int,
    maximum_hotspots: int,
) -> tuple[list[tuple[str, str, str]], dict[tuple[str, str], int]]:
    """기능 hotspot과 겹치지 않는 동의 변이 recurrent hotspot만 선택합니다."""
    support: Counter[tuple[str, str]] = Counter()
    for gene in columns:
        normalized = features[gene].astype("string").fillna("WT").str.strip().str.upper()
        for value in normalized[normalized.ne("WT")]:
    ...
    for rank, (gene, token) in enumerate(pairs, start=1):
        safe_token = SAFE_NAME_PATTERN.sub("_", token).strip("_") or "VARIANT"
        hotspots.append(
            (f"synonymous_hotspot_{rank:03d}_{gene}_{safe_token}", gene, token)
        )
    return hotspots, dict(support)
...
class EMV43PreprocessingPipeline(PreprocessingPipeline):
    """v15 outer OOF 중복을 제외하고 고유한 동의 변이 hotspot만 v24에 추가합니다."""
    ...
    def __init__(
        ...
        **parameters: object,
        ...
        self.synonymous_hotspots_: list[tuple[str, str, str]] = []
        self.synonymous_hotspot_support_: dict[tuple[str, str], int] = {}
        self.steps = (
            "변이 토큰 단일 분리 및 중복 제거",
            "전체 변이·기능 변이 채널 분리",
            "채널별 최소 빈도와 유전자 합집합",
            "단일 consequence summary와 기능·동의 변이 분리 hotspot",
    ...
    def _build_features(self, features: pd.DataFrame) -> pd.DataFrame:
        ...
        return pd.concat(
            [
                output,
                create_hotspot_features(
                    features, self.hotspots_ + self.synonymous_hotspots_
                ),
            ],
            axis=1,
        )

    def fit(self, features: pd.DataFrame, labels: pd.Series) -> "EMV43PreprocessingPipeline":
        ...
        (
            self.synonymous_hotspots_,
            self.synonymous_hotspot_support_,
        ) = learn_synonymous_hotspots(
            features,
            self.mutation_signature_genes_,
    ...
    def summary(self) -> dict[str, int]:
        ...
        result.update({
            ...
            "synonymous_hotspot_features": len(self.synonymous_hotspots_),
        })
        return result
