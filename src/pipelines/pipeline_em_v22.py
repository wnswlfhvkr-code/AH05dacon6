"""em_v16과 em_v26을 중복 없이 결합한 EM v22."""
...
# NCI GDC study name에서 유도한 모델링용 보조 그룹입니다.
# 원본 SUBCLASS를 병합하거나 하위 암종으로 분해하는 데 사용하지 않습니다.
TCGA_STUDY_FAMILIES = {
    "urothelial": ("BLCA",),
    "breast": ("BRCA",),
...
def add_consequence_summary(output: pd.DataFrame, severity: pd.DataFrame) -> None:
    """v16의 단일 burden과 consequence 구성 피처를 추가합니다."""
...
def map_families(labels: pd.Series) -> pd.Series:
    lookup = {
        label: family
...
def learn_all_weights(
    ...
    """동일 변이 행렬에서 class 및 family 가중치를 함께 학습합니다."""
    ...
    for class_name in sorted(aligned.dropna().unique()):
        ...
        class_weights[str(class_name)] = weights

    families = map_families(aligned)
    family_weights: dict[str, dict[str, float]] = {}
    for family in sorted(families.dropna().unique()):
        if len(TCGA_STUDY_FAMILIES[str(family)]) < 2:
            continue
        weights = stabilized_weights(
            mutation, families.eq(family), families.ne(family),
            top_genes_per_family, smoothing, shrinkage, max_log2_odds,
        )
        if not weights:
            raise ValueError(f"{family} 연구군 signature를 만들 수 없습니다.")
        family_weights[str(family)] = weights
    return class_weights, family_weights
...
def add_supervised_signatures(
    ...
    for family, weights in family_weights.items():
        ...
        output[f"tcga_family_{family}_match_rate"] = matrix.sum(axis=1) / burden
...
def create_oof_signatures(
    ...
    for train_positions, valid_positions in splitter.split(mutation, aligned):
        ...
        class_weights, family_weights = learn_all_weights(
            train, aligned.iloc[train_positions], top_genes_per_class,
            top_genes_per_family, smoothing, shrinkage, max_log2_odds,
        )
        fold_output = pd.DataFrame(index=valid.index)
        add_supervised_signatures(fold_output, valid, class_weights, family_weights)
        for column in fold_output:
            result.loc[valid.index, column] = fold_output[column]
    return result.astype("float32")


class EMV22PreprocessingPipeline(PreprocessingPipeline):
    """v16 severity/class/hotspot에 v26 family signature만 추가합니다."""
    ...
    def __init__(
        ...
        **parameters: object,
        ...
        self.steps = (
            ...
            "기능 결과 심각도와 구성",
            "공통 inner-fold OOF class/family signature",
            "학습 Fold 반복 hotspot",
        )
    ...
    def _build_features(self, features: pd.DataFrame) -> pd.DataFrame:
        ...
        add_supervised_signatures(
            output, mutation, self.class_gene_weights_, self.family_gene_weights_
        )
        return pd.concat(
            [output, create_hotspot_features(features, self.hotspots_)], axis=1
        )

    def fit(self, features: pd.DataFrame, labels: pd.Series) -> "EMV22PreprocessingPipeline":
        map_families(labels)
        ...
        self.class_gene_weights_, self.family_gene_weights_ = learn_all_weights(
            mutation, labels, self.top_genes_per_class, self.top_genes_per_family,
    ...
    def fit_transform(self, features: pd.DataFrame, labels: pd.Series) -> pd.DataFrame:
        ...
        oof = create_oof_signatures(
            ...
            self.top_genes_per_class, self.top_genes_per_family, self.smoothing,
    ...
    def summary(self) -> dict[str, int]:
        ...
        result.update({
            ...
            "tcga_family_signature_features": 2 * len(self.family_gene_weights_),
