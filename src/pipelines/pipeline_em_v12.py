"""기능 결과(v6)와 암종 signature(v9)를 결합한 EM 버전 12입니다."""
...
def select_genes_by_mutation_count(
    ...
    """v12 학습 데이터에서 최소 변이 횟수를 만족하는 유전자를 선택합니다."""
    mutation_counts = (
...
def create_consequence_severity_matrix(
    ...
    """v12 유전자 변이를 WT=0부터 truncating=4까지로 인코딩합니다."""
...
def add_consequence_summary_features(
    ...
    """v12의 결과 유형별 개수와 비율 및 변이 부담을 추가합니다."""
...
def learn_class_gene_weights(
    ...
    """v12 학습 fold에서 암종별 안정화 log2 odds 가중치를 학습합니다."""
...
def add_class_signature_features(
    ...
    """v12의 암종별 weighted/match signature를 추가합니다."""
    for class_name in class_names:
...
class EMV12PreprocessingPipeline(PreprocessingPipeline):
    """severity와 consequence를 유지하고 signature의 중복 burden은 제외합니다."""
    ...
    def __init__(
        ...
        **parameters: object,
        ...
        self.steps = ("최소 변이 빈도 필터", "기능 결과 심각도", "기능 결과 구성", "암종 signature")
    ...
    def fit(self, features: pd.DataFrame, labels: pd.Series) -> "EMV12PreprocessingPipeline":
    ...
    def summary(self) -> dict[str, int]:
        ...
        summary.update({"dropped_rare_features": len(self.dropped_rare_columns), "consequence_features": 9, "signature_features": 2 * len(self.class_names)})
        return summary
